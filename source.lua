--[[
	...
	Rayfield Interface Suite - Fork by NevercryNever
	Baseado no Rayfield original by Sirius (open source)

	MODIFICACOES:
	  - Tema "Vinho" adicionado
	  - secureNotify() silenciada (sem notificacoes de sistema)
	  - Analytics/telemetria removidos
	  - prompt de seguranca removido
	  - Icons carregados do fork proprio
...
local DISABLE_ALL_NOTIFICATIONS = true  -- false = Notify() funciona normalmente
-- ================================================

if debugX then
	warn('Initialising Rayfield Fork')
end
...
local requestsDisabled = true  -- analytics desativado no fork
...
-- secureNotify SILENCIADA — sem spam de notificacoes de sistema
local function secureNotify(wType, title, content)
	-- SILENCIADO INTENCIONALMENTE NO FORK
end
...
-- prompt de seguranca REMOVIDO do fork
local prompt = nil
if useStudio then
...
-- Analytics REMOVIDO — sem reporter, sem requests para servidores do Sirius
local reporter = nil

-- Prompt de seguranca REMOVIDO
-- (promptUser = 2 no original = desativado, mantemos sem bloco)
...
local RayfieldLibrary = {
	...
	Theme = {
		...
		Neon = {
		    ...
		    NotificationBackground = Color3.fromRGB(8, 8, 8),
		    NotificationActionsBackground = Color3.fromRGB(200, 200, 200),
		
		    TabBackground = Color3.fromRGB(12, 12, 12),
		    TabStroke = Color3.fromRGB(0, 255, 180, 0.15),
		    TabBackgroundSelected = Color3.fromRGB(0, 30, 20),
		    TabTextColor = Color3.fromRGB(160, 160, 160),
		    SelectedTabTextColor = Color3.fromRGB(0, 255, 180),
		
		    ElementBackground = Color3.fromRGB(10, 10, 10),
		    ElementBackgroundHover = Color3.fromRGB(18, 18, 18),
		    SecondaryElementBackground = Color3.fromRGB(7, 7, 7),
		    ElementStroke = Color3.fromRGB(0, 80, 55),
		    SecondaryElementStroke = Color3.fromRGB(0, 55, 38),
		
		    SliderBackground = Color3.fromRGB(0, 80, 55),
		    SliderProgress = Color3.fromRGB(0, 200, 140),
		    SliderStroke = Color3.fromRGB(0, 255, 180),
		
		    ToggleBackground = Color3.fromRGB(12, 12, 12),
		    ToggleEnabled = Color3.fromRGB(0, 210, 150),
		    ToggleDisabled = Color3.fromRGB(40, 40, 40),
		    ToggleEnabledStroke = Color3.fromRGB(0, 255, 180),
		    ToggleDisabledStroke = Color3.fromRGB(55, 55, 55),
		    ToggleEnabledOuterStroke = Color3.fromRGB(0, 140, 100),
		    ToggleDisabledOuterStroke = Color3.fromRGB(28, 28, 28),
		
		    DropdownSelected = Color3.fromRGB(0, 25, 18),
		    DropdownUnselected = Color3.fromRGB(10, 10, 10),
		
		    InputBackground = Color3.fromRGB(10, 10, 10),
		    InputStroke = Color3.fromRGB(0, 180, 120),
		...
		Vinho = {
		    ...
		    TabBackgroundSelected = Color3.fromRGB(160, 10, 20),
		    ...
		    ElementStroke = Color3.fromRGB(35, 8, 8),
		    SecondaryElementStroke = Color3.fromRGB(28, 6, 6),
		
		    SliderBackground = Color3.fromRGB(120, 8, 15),
		    SliderProgress = Color3.fromRGB(180, 12, 22),
		    SliderStroke = Color3.fromRGB(210, 15, 28),
		
		    ToggleBackground = Color3.fromRGB(14, 14, 14),
		    ToggleEnabled = Color3.fromRGB(185, 10, 20),
		    ToggleDisabled = Color3.fromRGB(45, 45, 45),
		    ToggleEnabledStroke = Color3.fromRGB(220, 15, 28),
		    ToggleDisabledStroke = Color3.fromRGB(60, 60, 60),
		    ToggleEnabledOuterStroke = Color3.fromRGB(140, 8, 15),
		    ToggleDisabledOuterStroke = Color3.fromRGB(30, 30, 30),
		
		    DropdownSelected = Color3.fromRGB(22, 6, 6),
		    DropdownUnselected = Color3.fromRGB(14, 14, 14),
		
		    InputBackground = Color3.fromRGB(14, 14, 14),
		    InputStroke = Color3.fromRGB(100, 8, 12),
...
if secureMode and not customAssetId then
	secureNotify("default_asset", "Secure Mode", "You are using the default Rayfield asset ID.")
end
...
do
	...
	if hasCustomAsset and hasFilesystem then
		...
		secureNotify("no_getcustomasset", "Rayfield", "Your executor does not support getcustomasset.")
	end
...
-- Icons carregados do SEU fork
local Icons = useStudio and require(script.Parent.icons) or loadWithTimeout(FORK_BASE_URL .. '/icons.lua')
...
function RayfieldLibrary:Notify(data)
	if DISABLE_ALL_NOTIFICATIONS then return end  -- SILENCIADO NO FORK
	task.spawn(function()
...
function RayfieldLibrary:CreateWindow(Settings)
	...
	if Settings.LoadingTitle ~= "Fabitos Interface Suite" then
		LoadingFrame.Version.Text = "Fabitos UI"
	end
	...
	if Settings.Discord and Settings.Discord.Enabled and not useStudio and not secureMode then
		...
		if not callSafely(isfile, RayfieldFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			...
			if Settings.Discord.RememberJoins then
				callSafely(writefile, RayfieldFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"Fabitos RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		end
	end
	...
	if (Settings.KeySystem) then
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("Fabitos | "..Key.." Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with Fabitos specific development.')
				end
			end
		end
		...
		if not Passthrough and secureMode then
			warn("Rayfield | Secure Mode: Key system requires a valid saved key.")
			Rayfield.Enabled = false
	...
	-- Analytics reporter REMOVIDO do fork — sem coleta de dados
...
function RayfieldLibrary:LoadConfiguration()
	...
	if CEnabled then
		...
		elseif not success and not notified then
			...
			RayfieldLibrary:Notify({Title = "Rayfield Configurations", Content = "We've encountered an issue loading your configuration file.", Image = 4384402990, Duration = 7})
		end
	end
...
function RayfieldLibrary:SetTheme(Theme)
	local success, result = pcall(ChangeTheme, Theme)
	if not success then
		RayfieldLibrary:Notify({Title = 'Unable to Change Theme', Content = 'We are unable to find that theme.', Image = 4400704299})
	end
end
